...
public class Joint {
	...
	public InterexchangeSeq CodeTransfDecode4Win(String seq, String errors){
		
		this.totInfoBit = seq.length();

		String[] s = errors.split(" ");
		...
		int currerr = 0;
		if(!s[0].equals(""))
			currerr = Integer.parseInt(s[0]);

		if(!s[0].equals("")) {
			for (int i = 0; i < codeString.length(); i++) {
				if (currerr == i) {
					if (errNumb != s.length) {
						currerr = Integer.parseInt(s[errNumb]);
						errNumb++;
					}
					if (Integer.parseInt(Character.toString(codeString.charAt(i))) == 0)
						sb.append(1);
					else
						sb.append(0);
				} else
					sb.append(Integer.parseInt(Character.toString(codeString.charAt(i))));
			}
		}
		else
			for (int i = 0; i < codeString.length(); i++) {
