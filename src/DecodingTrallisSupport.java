	...
	private class DecTrellisFrame extends JPanel{
		...
		synchronized protected void paintComponent(Graphics g){
			...
			synchronized(myTime){
				...
				//Iterator<StateAndInfoBit> itOnKeys =  refTrallis.myTrellis.keySet().iterator();
					...
					for(int i = 0;i <totStates; i++){
						if(myColumns.get(onTime).getColumn()[i].isActive() && myColumns.get(onTime).getColumn()[i].isActiveForGraph() ){
							//while(itOnKeys.hasNext()) {
								...
								//if(!itOnKeys.hasNext())
								//	key = refTrallis.myTrellis.keySet().iterator().next();
								stateFrom = myColumns.get(onTime).getColumn()[i].getFrom().getMyState();
								if (coordinatesL[stateFrom][onTime - 1][1] == 37 && coordinatesL[i][onTime][1] == 37)
									g2.setStroke(drawingStrokeDEF);
								if (coordinatesL[stateFrom][onTime - 1][1] == 412 && coordinatesL[i][onTime][1] == 412)
									g2.setStroke(drawingStrokeDSH);
								if (coordinatesL[stateFrom][onTime - 1][1] == 37 && coordinatesL[i][onTime][1] == 162)
									g2.setStroke(drawingStrokeDSH);
								if (coordinatesL[stateFrom][onTime - 1][1] == 162 && coordinatesL[i][onTime][1] == 287)
									g2.setStroke(drawingStrokeDEF);
								if (coordinatesL[stateFrom][onTime - 1][1] == 162 && coordinatesL[i][onTime][1] == 412)
									g2.setStroke(drawingStrokeDSH);
								if (coordinatesL[stateFrom][onTime - 1][1] == 287 && coordinatesL[i][onTime][1] == 37)
									g2.setStroke(drawingStrokeDEF);
								if (coordinatesL[stateFrom][onTime - 1][1] == 287 && coordinatesL[i][onTime][1] == 162)
									g2.setStroke(drawingStrokeDSH);
								if (coordinatesL[stateFrom][onTime - 1][1] == 412 && coordinatesL[i][onTime][1] == 287)
									g2.setStroke(drawingStrokeDEF);
							//}
