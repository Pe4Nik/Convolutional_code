...
public class DecTrallisColumn {
	...
	public DecoderOutput createWordSection(int codeWord){
		...
		while(it.hasNext() ){ 
			...
			fromFlow = (now = it.next()).getMyInflows(); 
								
			System.out.println("Parsing state " + now.getMyState().toString());				
			for(int i = 0; i<2;i++){ 
				
				from[i] = fromFlow[i].getMyState().getMyState().intValue(); 
				System.out.println("\t from " + Integer.toString(from[i]));
				
				if(prev.getColumn()[from[i]].isActive()){ 
					...
					if(this.column[stateCons].isActive()){ 
						if(tempMetric< this.column[stateCons].getMyWholeMetric()){
